      ...
      {isExpanded && (
        <CardContent className="space-y-6">
          ...
          {viewerRole !== 'client' && systemInformation?.quotesReceived && systemInformation.quotesReceived.length > 0 && (
            <div className="border-t pt-4">
              <h4 className="text-sm font-semibold text-foreground mb-3">Quotes Received ({systemInformation.quotesReceived.length})</h4>
              <div className="space-y-2">
                {systemInformation.quotesReceived.map((quote, index) => (
                  <div key={quote.quoteId || index} className="flex items-center justify-between bg-muted p-2 rounded">
                    <div>
