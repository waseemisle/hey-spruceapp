...
import { QuoteTimelineEvent, QuoteSystemInformation } from '@/types';
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card';
import { Button } from '@/components/ui/button';
import { ChevronDown, ChevronUp, Clock, FileText, CheckCircle, XCircle, Send } from 'lucide-react';

interface QuoteSystemInfoProps {
...
export default function QuoteSystemInfo({ timeline, systemInformation, creationSourceLabel, canViewTimeline = true }: QuoteSystemInfoProps) {
        ...
        <CardContent className="space-y-6">
          {creationSourceLabel && (
            <div className="bg-blue-50 border border-blue-100 rounded-lg p-3">
              <p className="text-xs font-medium text-blue-800 mb-0.5">How this quote was created</p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              {systemInformation.acceptedBy && (
                <div className="bg-green-50 p-3 rounded-lg">
                  ...
                  <p className="text-sm font-medium text-green-900">{systemInformation.acceptedBy.name}</p>
                  <p className="text-xs text-green-600">{formatTimestamp(systemInformation.acceptedBy.timestamp)}</p>
                </div>
              )}
              {systemInformation.rejectedBy && (
                <div className="bg-red-50 p-3 rounded-lg">
                  <p className="text-xs text-red-700 mb-1">Rejected By</p>
                  <p className="text-sm font-medium text-red-900">{systemInformation.rejectedBy.name}</p>
                  <p className="text-xs text-red-600">{formatTimestamp(systemInformation.rejectedBy.timestamp)}</p>
                </div>
              )}
            </div>
          )}
                      ...
                      <div className="flex items-start justify-between gap-2">
                        <div className="flex-1">
                          ...
                          <p className="text-sm text-gray-600 mt-0.5">
                            {event.details || (event.type === 'created' ? 'Quote created' : '')}
                          </p>
