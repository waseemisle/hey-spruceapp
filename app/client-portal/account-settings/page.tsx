...
export default function ClientAccountSettings() {
  ...
  useEffect(() => {
    ...
    const unsub = onAuthStateChanged(auth, async (firebaseUser) => {
      if (!firebaseUser) {
        // Guard against transient null. Real logout: auth.currentUser is
        // also null and we redirect; otherwise the null arg is just Firebase
        // restoring the persisted user, ignore and wait for next callback.
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      ...
      const savePromise = setDoc(doc(db, 'clients', uid), {
      ...
      <div className="max-w-4xl mx-auto space-y-4 sm:space-y-5 pb-16 p-4 sm:p-6">
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="bg-card rounded-xl border border-border p-4 sm:p-6">
              <div className="flex items-center gap-3 mb-6"><User className="h-5 w-5 text-primary" /><h2 className="text-base font-semibold text-foreground">Personal Information</h2></div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="relative">
                    ...
                    <Input id="companyName" value={companyName} disabled className="pl-9 bg-muted text-muted-foreground cursor-not-allowed" />
                  </div>
